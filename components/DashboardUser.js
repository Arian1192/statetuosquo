...
const DashboardUser = () => {
    ...
    return (
        <div className="flex ">
            <div className={`h-screen ${isOpen ? "w-64" : "w-20"} duration-200  border-2 border-zinc-400 bg-zinc-200 rounded-tr-2xl dark:bg-slate-500 dark:border-t-emerald-400 dark:border-r-emerald-400 dark:border-l-0  text-black relative`}>
                {data && <h2 className={`${!isOpen && 'scale-0'} duration-300 text-lg font-semibold text-center text-zinc-700 mt-7`}>{loading && 'loading...'}{data && data.username}</h2>}
                ...
                <div className="flex flex-col mt-10">
                    <div className="flex items-center justify-center ">
                        <ul className="flex flex-col w-full gap-5 pl-4 text-lg text-zinc-800">
                            <li className="flex mb-5">
                                {isOpen && <span className="ml-3">Twitter</span>}
                                <BsTwitter className="mt-1 ml-3 text-2xl cursor-pointer hover:text-sky-500 hover:animate-wave" />
                            </li>
                            <li className="flex mb-5">
                                {isOpen && <span className="ml-3">Instagram</span>}
                                <div className="flex items-center justify-start p-1 ml-2 rounded-lg cursor-pointer hover:bg-gradient-to-br from-purple-700 via-red-600 to-yellow-500 hover:text-white hover:animate-wave ">
                                    <BsInstagram className="inline-block text-2xl" />
                                </div>
                            </li>
                            <li className="flex mb-5">
                                {isOpen && <span className="ml-3">Twitch</span>}
                                <BsTwitch className="mt-1 ml-3 text-2xl cursor-pointer hover:text-fuchsia-800 hover:animate-wave" />
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div className="flex-1 h-screen text-2xl font-semibold p-7 text-zinc-700 ">
                {data && `Main Home Page of ${data.name}`}
